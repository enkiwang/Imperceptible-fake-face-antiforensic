...
parser.add_argument('--max_epsilon1', type=float, default=2.5, help='maximum perturbation at Y.')
parser.add_argument('--max_epsilon2', type=float, default=6, help='maximum perturbation at Cb.')
parser.add_argument('--max_epsilon3', type=float, default=6, help='maximum perturbation at Cr.')
args = parser.parse_args()

os.environ['CUDA_VISIBLE_DEVICES'] = args.gpu_id
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

output_dir = args.output_dir + 'src_m' + str(args.model_num) + '_eps_' + str(args.max_epsilon1) + str(args.max_epsilon2) + str(args.max_epsilon3) + '_ens_123/' #an example of models to ensemble
...
model1 = load_model(args.ckpt_fakenet, FakeNet)
model2 = load_model(args.ckpt_vggnet, VGGNet)
model3 = load_model(args.ckpt_discnet, DiscNet)
...
def attack_ens(img_ycc, label, model, eps, mu=1.0, iterMax=args.num_iter):
    ...
    for k in range(iterMax):
        ...
        output = (models['model1'](img_rgb_k) +  models['model2'](img_rgb_k)  +
                    models['model3'](img_rgb_k) ) / 3.0   # an example of models to ensemble
...
for img_idx, img_path in enumerate(img_lists):
    ...
    img_adv = attack_ens(img_ycc, img_label, model, epsilon)  
